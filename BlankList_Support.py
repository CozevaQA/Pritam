...
def BlankList_Support_Script(CUSTOMER_ID, QUARTER, COUNTER):
    """
    ********************************************************* BLANK SUPPORT VALIDATION SCRIPT *********************************************************
    """
                ...
                **** SUPPORT MEASURE REGISTRY NAVIGATION ****
                ...
                while measure_counter < len(measures_all) and score < len(scores):
                    ...
                    try:
                        ...
                        WebDriverWait(driver, 120).until(EC.presence_of_element_located((By.XPATH, "//a[@class='breadcrumb']")))
                        driver.find_element_by_xpath("//a[@class='breadcrumb']").click() 

                        # """PRACTICES LIST"""
                        # driver.find_element_by_xpath("(//ul[@id='qt-mt-support-ls']/li)[1]").click()
                        # ajax_preloader_wait()
                        # pracs = driver.find_element_by_xpath("//div[@id='metric-support-prac-ls_info']").text

                        # print(quarter_name + "~" + lob_name + "~" + measure_name + "~" + str(provs) + "~" + str(pats) + "~" + str(pracs))
                        ...
                        sh1.append((quarter_name + " | " + lob_name, measure_name, Numerator,Denominator, '', '', '', 'FAILED', 'Exception occurred!', providers_list_url))
                        driver.save_screenshot(path2 + "\\" +quarter_name + " "+ lob_name + "_"+ measure_name+"_Exception" +  ".png")
                ...
                lobs = driver.find_elements_by_xpath("//ul[@id='filter-lob']/li[@class!='hide']") #//ul[@id='filter-lob']/li
...
BlankList_Support_Script(200, 3, 5)
